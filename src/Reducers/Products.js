import {
  ...
  Did_Upload_Product,
  Clicked_Product_ID,
  WILL_REQUEST_CATEGORY_PRODUCT,
  DID_REQUEST_CATEGORY_PRODUCT,
  WILL_GET_BUYING_PRODUCT,
  DID_GET_BUYING_PRODUCT,
  WILL_GET_SELLING_PRODUCT,
  DID_GET_SELLING_PRODUCT,
  WILL_GET_PROFILE_STATS,
  DID_GET_PROFILE_STATS,
  ERROR_UPLOAD_PRODUCT
} from "../Actions/productsAction";
import { mockProducts } from "./mockData";

export default function Products(
  state = { clickedProductID: -1, products: mockProducts },
  action
) {
  switch (action.type) {
    ...
    case FILTER_PRODUCTS:
      ...
      for (var index = 0; index < state.initialProducts.length; index++) {
        if (
          state.initialProducts[index].description.indexOf(action.searchTerm) !=
            -1 ||
          state.initialProducts[index].name.indexOf(action.searchTerm) != -1
        ) {
